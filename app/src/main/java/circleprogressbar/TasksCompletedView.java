...
private void initAttrs(Context context, AttributeSet attrs) {
    ...
    mCircleColor = typeArray.getColor(
            R.styleable.TasksCompletedView_circleColor, 0xFFF4F4F4);
    mRingColor = typeArray.getColor(
            R.styleable.TasksCompletedView_ringColor, 0xFF4EBBEC);

    mRingRadius = mRadius + mStrokeWidth / 2; // 为什么加二分之一的圆环宽度
}
...
private void initVariable() {
    ...
    mTextPaint.setColor(0xFF303030);
...
public void setProgress(int progress) {
    mProgress = progress;
    // invalidate();
    postInvalidate(); // 刷新界面 在工作者线程中被调用 在线程中调用
}
